...
function handleKeyDown(event: KeyboardEvent) {
  if (!dropdownIsVisible.value || searchResults.value.length === 0) return;
...
<template>
  <div class="station-search-input relative w-full max-w-xl">
    <div class="w-full px-4 py-2 rounded-full border border-gray-300 dark:border-gray-600
      ...
      <input
          ...
          class="flex-grow bg-transparent focus:outline-none text-sm text-gray-900 dark:text-gray-100 placeholder-gray-400"
          @input="updateResults"
          @click="updateResults"
      ...
      <button
          @click="tryStop"
          class="ml-2 px-4 py-1.5 bg-orange-500 hover:bg-orange-600 text-white rounded-full text-sm font-medium"
      >
        Essayer
      </button>
    </div>
