...
defineProps(['addedStations', 'toggleStop']);
...
<template>
  <div v-if="addedStations.length > 0" class="space-y-4">
    ...
    <TransitionGroup name="list" tag="ul" class="space-y-3">
      <li
        v-for="(stop, index) in addedStations"
        :key="index"
      >
        <button
          class="w-full text-left flex items-center px-3 py-1 rounded-full text-sm transition hover:bg-blue-200 dark:hover:bg-blue-800"
          :class="[
            stop.isActive
